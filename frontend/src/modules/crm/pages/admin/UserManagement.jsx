...
async function updateProfileApi(userId, profileData) {
  ...
  const response = await axios.put(`${VITE_API_BASE_URL}/profiles/${userId}`, payload, {
    headers: { Authorization: `Bearer ${authKey}` },
  });
  return response.data;
}
...
const validateUserForm = (data = {}) => {
  const newErrors = {};
  if (!data.name || !String(data.name).trim()) newErrors.name = 'Full name is required';
  if (!data.email || !String(data.email).trim()) newErrors.email = 'Email is required';
  ...
  if (data.isEdit) {
    ...
    if (!data.password || !String(data.password).trim()) newErrors.password = 'Password is required';
...
export default function UserManagement() {
  ...
  const handleEditOpen = (user) => {
    setEditUser({ ...user, password: '', isEdit: true });
    setShowPassword((prev) => ({ ...prev, edit: false }));
    setEditErrors({});
    setEditOpen(true);
  };
  ...
  const handleEditChange = (e) => {
    const { name, value } = e.target;
    if (name === 'status') {
      setEditUser((prev) => ({ ...prev, isActive: value === 'Active' }));
    } else {
      setEditUser((prev) => ({ ...prev, [name]: value }));
    }
  };
  ...
  const handleEditSave = async () => {
    ...
    // Validate edit form (password optional)
    const validationErrors = validateUserForm({ ...editUser, isEdit: true });
  ...
  const submitCreateProfile = async () => {
    const validationErrors = validateProfileForm(createProfileData);
    if (Object.keys(validationErrors).length > 0) {
  ...
  const handleEditExtraChange = (event) => {
    const { name, value } = event.target;
    setEditExtraUser((prev) => ({ ...prev, [name]: value }));
    setEditProfileErrors((prev) => ({ ...prev, [name]: undefined }));
  };

  const submitUpdateProfile = async () => {
    try {
      if (!currentUserId) throw new Error('Missing userId for profile update');
      const validationErrors = validateProfileForm(editExtraUser);
      if (Object.keys(validationErrors).length > 0) {
            ...
            <TableBody>
              {loading ? (
                ...
                usersData.map((user) => (
                  <TableRow key={user.userId} hover>
                    <TableCell>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>{getRoleIcon(user.role?.name || user.role)} {user.name}</Box>
                    </TableCell>
              ...
              {Object.entries(editExtraUser)
                ...
                .map(([key, value]) => (
                  <TextField
                    ...
                    fullWidth
                    onChange={handleEditExtraChange}
                    error={!!editProfileErrors[key]}
                    helperText={editProfileErrors[key]}
                  />
                ))}
            </Stack>
          ) : (
        ...
        <Dialog open={editOpen} onClose={handleEditClose} maxWidth="sm" fullWidth>
          ...
          <DialogContent>
            <Stack spacing={2} sx={{ mt: 2 }}>
              ...
              />
              <FormControl fullWidth error={!!editErrors.role}>
                <InputLabel>Role</InputLabel>
                <Select name="role" value={editUser.role?.name || editUser.role || ''} label="Role" onChange={handleEditChange}>
                  {roleOptions.map((role) => <MenuItem key={role} value={role}>{role}</MenuItem>)}
                </Select>
                {editErrors.role && <Typography variant="caption" color="error">{editErrors.role}</Typography>}
              </FormControl>
              <FormControlLabel
                control={(
                  <Switch checked={!!editUser.isActive} onChange={(e) => setEditUser((prev) => ({ ...prev, isActive: e.target.checked }))} />
                )}
